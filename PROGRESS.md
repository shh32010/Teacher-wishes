...
**项目启动**：2026-08-05 | **v2.0 上线**：2026-09-06 | **当前阶段**：v2.0.0 已上线（活动运行中，至 09-12）

---

## 📊 当前状态

**版本**：v2.0.0（生产运行中） | **v1.3.4** 已发布（历史版本） | **v2 设计蓝图**：`docs/V2_DESIGN.md`（2026-08-29 定稿）

### v2.0 里程碑

| 里程碑 | 内容 | 状态 |
| :--- | :--- | :---: |
| M1 数据层 | 011/012/013 迁移（词库/礼物/ai_generations 新表 + blessings ALTER + 严格触发器）+ 类型 + 60 条测试词库 | ✅ |
| M2 后台运营 | 词库 CRUD + CSV 导入 + 礼物管理 + 管理后台 3 个新 tab | ✅ |
| M3 送礼流程 | /gift 6 步状态机 + 礼物动画 + POST 新契约（服务端取词）+ 4 个公开 API | ✅ |
| M4 展示层 | GiftGalaxy 礼物芯河 + 祝福墙礼物/情绪标签 + 首页 CTA/今日金句 + **大屏删除**（用户拍板） | ✅ |
| M5 AI 全量 | DeepSeek adapter（用户拍板）+ 批量分类/金句/总结/洞察 + 规则降级 + 仪式文案矩阵 | ✅ |
| M6 测试收口 | 107 单测 + E2E 25 用例 + 安全回归 21 项 + k6 v2 契约 + 文档同步 | 🔄 收尾中 |
| M7 发布 | 013 触发器执行 + Vercel 部署 + AI_API_KEY 配置 + 生产冒烟 + 词库导入验收 | ⬜ |

### v2.0 核心决策（已确认）

- 上线 **09-05**；学生**允许多次**送礼（冷却+限流）；**保留昵称/班级可选**
- 自由输入完全移除（纯词库选择）；历史 353 条祝福保留展示
- AI 供应商 **DeepSeek**；**大屏模式整体删除**；teachers 表保留（介绍页）
...
> ⚠️ 本表为 v1.3.3 发布时的历史快照，部分事项已随 v2.0 演进失效（大屏已删除、敏感词已接入、截图已更新为 v2）。

| # | 事项 | v1 状态 | v2.0 现状 |
| :--- | :--- | :---: | :--- |
| 1 | 截图更新 | ✅ | ✅ v2 版已更新（删除大屏截图） |
| 2 | 移动端真机走查 | ⬜ | 🔄 待 v2.0 发布前走查 |
| 3 | 后台添加教师 UI | ⬜ | ⬜ 仍待开发 |
| 4 | 敏感词过滤 | ⬜ | ✅ v1.3.4 完成，v2 用于词库入库/提交双保险 |
| 5 | Lighthouse 审计 | ⬜ | ⬜ 待 v2.0 发布后跑 |
| 6 | 分支预览环境 | ⬜ | ⬜ 待配置 |
| 7 | 大屏长时间稳定性 | ⬜ | ~~大屏已删除~~（v2 决策 11） |
| 8 | 粒子自适应 | ⬜ | ⬜ 可延后 |
| 9 | 敏感词库扩充 | ⬜ | ✅ v1.3.4 完成 |
| 10~15 | 环境变量/冒烟/压测/Cron/密钥/SEO | ✅ | ✅ 沿用 |
...
| # | 描述 | 严重程度 | 状态 |
| :--- | :--- | :--- | :--- |
| 1 | ~~管理后台假登录~~ → 纯密码 + admin_token HMAC 鉴权（中途的 Supabase Auth 方案已废弃） | 🟢 | ✅ 已修复 |
...
| 8 | ~~敏感词过滤待接入~~ → v1.3.4 已接入（中英文词库），v2.0 用于词库入库/提交双保险 | 🟢 | ✅ v1.3.4 |
...
- ✅ 芯河教师天体：教师头像展示在芯河外层（蓝色光晕），祝福星星在内层
- ✅ 芯河弹窗预览：点击教师天体/祝福星星弹出详情弹窗，不脱离首页
...
- ✨ 点赞爱心爆发特效（LikeBurst）+ 祝福芯河辉光增强（三层 shadow）
...
### 08-29（v2.0 改版）
- 📋 **甲方新需求**：祝福语由甲方收集供学生选择 / 增加送礼物效果 / 取消指定老师（防数据不均）
- 📄 **设计文档**：`docs/V2_DESIGN.md` 定稿（产品定位「AI 沉浸式教师节互动送礼平台」+ 8 个 AI 创意点 + 安全红线）
- ✅ 决策确认：9-05 上线、允许多次送礼、保留昵称班级、AI 用 DeepSeek、**大屏模式整体删除**
- ✅ M1 数据层：011 新表迁移 + 012 测试词库 60 条 + 013 严格触发器 + 类型定义
- ✅ M2 后台运营：词库 CRUD + CSV 导入（RFC 4180 解析器）+ 礼物管理 + AI 中心
- ✅ M3 送礼流程：/gift 6 步状态机 + 8 种礼物动画 + POST 服务端取词契约 + Turnstile Hook
- ✅ M4 展示层：GiftGalaxy 礼物芯河（中心 TEACHERS 光核）+ 祝福墙礼物标签 + 首页 CTA 改造
- ✅ M5 AI 全量：DeepSeek adapter + 批量分类/金句打分/收官总结 + 规则降级 + 48 组仪式文案矩阵
- ✅ M6 测试：单元 107/107 + E2E 25/25（新增 gift.spec）+ 安全回归扩展 6 项 + k6 v2 契约
- 🐛 修复：CSV 解析器字段内引号处理（RFC 4180 对齐，测试驱动发现）
- 🐛 修复：E2E motion 按钮坐标点击漂移（入场动画期间 force click 策略统一）
