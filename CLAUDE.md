...
│   │   ├── ai/                    # v2.0 AI 层（provider adapter + prompts + 仪式文案矩阵）
│   │   ├── auth/
...
│   │   └── useTurnstile.ts        # v2.0 Turnstile Hook（widget 生命周期管理）
│   ├── types/
...
| `/api/admin/templates` | GET/POST/PATCH/DELETE | Admin | requireAdmin + CSRF | 词库 CRUD（被引用仅停用） |
| `/api/admin/templates/import` | POST | Admin | requireAdmin + CSRF | CSV 批量导入（≤2MB/≤1000 行） |
| `/api/admin/gifts` | GET/PATCH | Admin | requireAdmin + CSRF | 礼物管理（字段白名单） |
| `/api/admin/ai/classify` | POST | Admin | requireAdmin + CSRF | 批量 AI 分类（无 key 规则降级） |
| `/api/admin/ai/quotes` | POST/PATCH | Admin | requireAdmin + CSRF | 金句候选打分 / 确认上线 |
| `/api/admin/ai/summary` | POST | Admin | requireAdmin + CSRF | 收官总结生成 |
| `/api/admin/login` | POST | Anon | CSRF + 限流 | 管理员登录 |
| `/api/admin/logout` | POST | Admin | 无 | 清除 admin_token |
| `/api/admin/upload` | POST | Admin | requireAdmin + CSRF | 头像上传 |
| `/api/csrf` | GET | 无 | 无 | CSRF 令牌生成 |
| `/api/cron/cleanup` | GET | Cron | CRON_SECRET | 限流记录定时清理 |
...
TURNSTILE_SECRET_KEY=               # Turnstile 密钥（未配置返回 503）

# AI（v2.0，可选 — 未配置时所有 AI 功能规则降级）
AI_PROVIDER=deepseek                # deepseek（默认）| zhipu | siliconflow
AI_API_KEY=                         # AI 平台 API key（仅服务端）
AI_MODEL=                           # 可选，覆盖默认模型（deepseek-chat）
...
- **懒加载**：tsParticles、Canvas Confetti、QuoteOfDay、管理面板均使用 `next/dynamic`
...
- **v2.0 送礼状态机**：`GiftFlow` 6 步（emotion→blessing→gift→confirm→sending→success），提交在 confirm 步完成，sending 仅为动画呈现，动画失败不影响提交状态
- **v2.0 内容安全**：客户端只传 `template_id`，服务端查词库取 content（客户端伪造 content 无效）；停用模板/礼物因 RLS 自动查不到 → 400
- **v2.0 AI 原则**：活动期间**零实时 LLM 调用** — 学生端推荐 = DB tags 查询；LLM 仅低频后台任务（分类/金句/总结）；无 `AI_API_KEY` 时全部规则降级；AI 故障不影响送礼核心链路
- **v2.0 仪式文案**：`lib/ai/messages.ts` 静态矩阵（6 情绪 × 8 礼物 = 48 组），POST 时取快照写入 `ai_message`
- **v2.0 Turnstile**：`useTurnstile(active)` hook 管理 widget 生命周期（保存 id 防重复 render、卸载时 remove 防悬空引用）
