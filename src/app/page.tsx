...
// 今日金句（懒加载，无金句时自动隐藏）
const QuoteOfDay = dynamic(() => import('@/components/ai/QuoteOfDay'), {
  ssr: false,
});

const GiftGalaxy = dynamic(() => import('@/components/home/GiftGalaxy'), {
  ssr: false,
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden">
      ...
      <div className="fixed right-4 top-4 z-20 flex flex-col items-end gap-2">
        <ThemeToggle />
        {/* 回访用户跳过时间线动画直达 CTA（UX-3） */}
        {!isActive('button') && (
          <button
            onClick={() => setStage('button')}
            className="rounded-full bg-ink/10 px-3 py-1 text-xs text-ink-muted backdrop-blur-sm hover:text-ink transition-colors"
            aria-label="跳过动画直达按钮"
          >
            跳过动画 →
          </button>
