...
// ---- 每个 VU 初始化一次 ----
// 获取词库模板与礼物列表，供后续 POST 送礼请求随机选择
let templates = [];
let gifts = [];

export function setup() {
  const t = http.get(`${BASE_URL}/api/templates?pageSize=50`);
  if (t.status === 200) {
    templates = t.json().data || [];
  }
  const g = http.get(`${BASE_URL}/api/gifts`);
  if (g.status === 200) {
    gifts = g.json().gifts || [];
  }
  console.log(`[setup] 已加载 ${templates.length} 条词库 / ${gifts.length} 种礼物`);
  return { templates, gifts };
}

// ---- 主测试逻辑 ----
export default function (data) {
  // 收集 template/gift IDs（从 setup 阶段传入）
  const templateIds = (data?.templates || []).map((t) => t.id);
  const giftIds = (data?.gifts || []).map((g) => g.id);
  ...
  if (route < 0.4) {
    ...
    // ============================================================
    // 10%：提交祝福 + 送礼 POST（v2.0 契约）
    // ============================================================
    group('提交祝福', () => {
      if (templateIds.length === 0 || giftIds.length === 0) return;

      // CSRF：GET /api/csrf（k6 自动管理 per-VU cookie jar）
      ...
      const payload = {
        ...
        gift_id: giftIds[Math.floor(Math.random() * giftIds.length)],
