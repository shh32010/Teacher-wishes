...
| 内容安全 | 敏感词过滤 + 人工审核 | 官方词库 + 敏感词过滤 + 审核（双保险） |
...
**参与规则（已确认）**：允许一个学生多次送礼，每轮完整流程算一次。提交成功后 3 秒冷却 + 服务端 IP 限流（每 10 分钟 3 条，沿用 v1 阈值）双保险，无需强唯一约束。
...
`teachers` 表**保留**：教师介绍页 `/teacher/[id]` 继续展示历史祝福；未来可重新扩展，但 v2.0 公开页不展示教师维度数据对比。
...
| `/teacher/[id]` | 保留 | 教师介绍 + 历史祝福（v2.0 新祝福不进入） |
