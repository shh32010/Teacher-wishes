...
**v2.0 送礼提交**：提交祝福 + 礼物（内容=官方词库原文，数据库触发器强制 `approved` **自动上墙**，事后治理走后台删除）。客户端只传模板/礼物 ID，**祝福内容由服务端从官方词库读取**——客户端伪造 content 无效。**v2.1：礼物可选**——缺省 `gift_id` = 只送祝福（前端「跳过礼物」按钮）。
...
```json
{
  ...
  "gift_id": "rose (可选, 礼物slug, 缺省=只送祝福)",
...
| gift_id 非法 slug（提供时才校验） | `非法礼物ID` |
| 模板不存在或停用（RLS 过滤） | `祝福语不存在或已停用` |
| 礼物不存在或停用（RLS 过滤，提供 gift_id 时） | `礼物不存在或已停用` |
...
> 🔒 服务端用 anon client 查模板/礼物，RLS 只返回 `is_active=true` 的行——停用的词库/礼物对攻击者天然不可用。插入时 `teacher_id` 恒为 null（v2.0 取消指定老师）、`emotion` 取模板分类快照、`ai_message` 取仪式文案矩阵快照（无礼物时降级通用文案）、`gift_id` 为 null 表示跳过礼物。
