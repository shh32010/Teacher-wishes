...
| 操作 | 限制 | 窗口 | 说明 |
| :--- | :--- | :--- | :--- |
| 提交祝福 | 400 次/IP | 10 分钟 | POST /api/blessings（校园 NAT 场景放宽阈值） |
| 提交祝福（全局） | 600 次/全站 | 1 分钟 | 第二道防线：防多 IP 分布式刷量打爆数据库（固定桶 `ip='global'`） |
| 点赞 | 20 次/IP | 1 分钟 | POST /api/blessings/[id]/like |
| 管理登录 | 5 次/IP | 1 分钟 | POST /api/admin/login |

> **说明**：IP 限流主要用于异常流量抑制；Turnstile 是提交祝福的人机验证主防线；全站桶防僵尸网络换 IP 绕过单 IP 阈值。限流数值已针对校园 NAT 共享 IP 场景放宽，全站 600/分钟（≈10/s）远高于活动实测峰值，正常场景不误伤。
